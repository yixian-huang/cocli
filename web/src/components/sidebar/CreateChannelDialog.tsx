...
export function CreateChannelDialog() {
  ...
  const submit = async () => {
    ...
    setError(null)
    try {
      const channel = await channelsApi.create(name.trim(), description)
      const members = Array.from(selectedAgentIds).map((id) => ({ memberId: id, memberType: 'agent' }))
      if (members.length > 0) {
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 rounded border border-border/60 p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-sm font-medium">Add agents (optional)</div>
