...
> Gemini, Kimi, Grok, Chatrs, and OpenCode agents in a Slack-like workspace on
> your own machine. No cloud, no signup, no data leaves your laptop.
...
runtime/driver layer. The production runtime set is Claude, Cursor, Codex,
Gemini, Kimi, Grok, Chatrs, and OpenCode. `cocli-cloud/daemon-rs` consumes the
versioned OSS crates at an exact revision and does not keep parallel runtime
implementations. Cloud-only remote connection, tenant authentication,
Postgres, quota, billing, and operations code stay outside this repository.
The repositories do not auto-merge: shared fixes land upstream here and cloud
upgrades an explicit revision or release. See
